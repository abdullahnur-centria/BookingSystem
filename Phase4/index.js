...
const app = express();
// FIX 1: Corrected IPORT typo so the server actually listens on the right port
...
const resourceValidators = [
  ...
  body('resourcePriceUnit')
    ...
    .trim()
    // Minor Fix: Added missing 'week' and 'month' to match the error message
...
app.post('/api/resources', resourceValidators, async (req, res) => {
  ...
  // FIX 4: Removed the line `resourceAvailable = false;` which was hardcoding/overriding user input
  ...
  try {
    ...
    `;
    const params = [
      resourceName, // FIX 5: Removed crypto hash to save the actual name instead of a hash
      resourceDescription,
      Boolean(resourceAvailable),
      Number(resourcePrice), // FIX 6: Removed "*2" to save the actual price instead of doubling it
      resourcePriceUnit
    ];
